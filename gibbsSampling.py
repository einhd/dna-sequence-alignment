...
def goToGibbs(motifL):
    print(motifL)
    main(motifL)

def main(motifL):

    intMotifLength = int(motifL)
    N=100     #Yineleme sayısı
    with open("uploads/dnaSequence1.txt", "r", encoding="utf-8") as file:
        sequences = [line.strip() for line in file]
        sequenceLength = len(sequences[0])

    bestMotif = multipleSeedsGibbsSampling(sequences, sequenceLength, intMotifLength, N)
    bestScore = score(bestMotif)
    print(bestMotif)
    print(bestScore)

    with open("outputs/motifFindingOutput.txt", "w", encoding="utf-8") as file:

        file.write(" \n")
        file.write("Aranan motif uzunlugu :" + str(intMotifLength) + " \n\n")
        file.write("Motifs: \n")
        for i in range(len(bestMotif)):
            file.write("[" + bestMotif[i] + "], \n")
        file.write("\nYineleme Sayisi = " + str(N))
        file.write("\nMismatch Count (Best Score) = " + str(bestScore) +"\n")
